...
import { HeritageClauseReflection } from '../../doc/ast/interface'
...
export default class InterfaceHeritage<P extends InterfaceHeritageProps> extends React.Component<
	P,
	InterfaceHeritageState
> {
						...
						<div key={clause.selfRef.id} className={block('heritage')}>
							...
							<span className={block('heritage-types')}>
								{clause.types.map(expr => {
									return <TypeExpression key={expr.selfRef.id} expr={expr} />
								})}
							</span>
						</div>
					)
