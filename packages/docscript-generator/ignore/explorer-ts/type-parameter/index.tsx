...
import { TypeParameterReflection } from '../../doc/ast/type/type-parameter'
import { isTypeReferenceReflection } from '../../doc/ast/type/type-reference'
import { isExpressionWithTypeArgumentsReflection } from '../../doc/ast/type/expression'
...
export default class TypeParameter extends React.Component<TypeParameterProps, TypeParameterState> {
	...
	render() {
		...
		return (
			<Paper id={typeParam.selfRef.id} className={this.getClassName()}>
				<SmartLink key="link" route={route}>
					{typeParam.name}
				</SmartLink>
				{this.props.asConstraint && typeParam.constraint && this.renderConstraint()}
			</Paper>
		)
	}
	...
	renderConstraint() {
		...
		return [<span key="extends"> extends </span>, view]
	}
}
