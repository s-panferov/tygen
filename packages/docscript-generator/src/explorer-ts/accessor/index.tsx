...
export default class Accessor extends React.Component<AccessorProps, AccessorState> {
	...
	render() {
		...
		return (
			<Panel
				...
				{method.comment && <Comment comment={method.comment} />}
			</Panel>
		)
	}
	...
	getInfo(): [GetAccessorDeclarationReflection | SetAccessorDeclarationReflection, string] {
		...
		let type = this.props.getter ? 'get' : 'set'

		return [method, type]
	}

	renderSignature(
		method: GetAccessorDeclarationReflection | SetAccessorDeclarationReflection,
		type: string
	): React.ReactNode {
		return [type, ' ', <Signature typeStyle={SignatureTypeStyle.Colon} signature={method} />]
	}
}
