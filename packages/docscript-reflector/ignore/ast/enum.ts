import { Declaration, EnumDeclaration, EnumMember, SyntaxKind } from 'typescript'
...
export function isEnumDeclaration(statement: Declaration): statement is EnumDeclaration {
	return statement.kind === SyntaxKind.EnumDeclaration
}

export function visitEnum(en: EnumDeclaration, ctx: Context): EnumDeclarationReflection {
	return ctx.dive(en, () => {
		...
		return Object.assign(basicInfo, {
			...
			members: en.members && en.members.map(member => visitEnumMember(member, ctx))
		})
	})
}
