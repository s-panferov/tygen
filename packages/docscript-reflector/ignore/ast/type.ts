...
import { visitPropertySignature, visitPropertyDeclaration } from './type/property'
...
import { visitUnionType, visitIntersectionType } from './type/intersection-union'

import { visitTypeReference } from './type/type-reference'

import { visitStringLiteralTypeNode } from './type/string-literal'
...
export function visitDeclarations(members: NodeArray<Declaration>, ctx: Context): Item[] {
	let reflections: Item[] = []

	for (let [, member] of members.entries()) {
		if (nodeIs.isPropertySignature(member)) {
			reflections.push(visitPropertySignature(member, ctx))
		} else if (nodeIs.isIndexSignatureDeclaration(member)) {
			reflections.push(visitIndexSignature(member, ctx))
		} else if (nodeIs.isCallSignature(member)) {
			reflections.push(visitCallSignature(member, ctx))
		} else if (nodeIs.isMethodSignature(member)) {
			reflections.push(visitMethodSignature(member, ctx))
		} else if (nodeIs.isPropertyDeclaration(member)) {
			reflections.push(visitPropertyDeclaration(member, ctx))
		} else if (nodeIs.isMethodDeclaration(member)) {
			reflections.push(visitMethodDeclaration(member, ctx))
		} else if (nodeIs.isConstructorDeclaration(member)) {
			reflections.push(visitConstructorDeclaration(member, ctx))
		} else if (nodeIs.isGetAccessorDeclaration(member)) {
			reflections.push(visitGetAccessorDeclaration(member, ctx))
		} else if (nodeIs.isSetAccessorDeclaration(member)) {
			reflections.push(visitSetAccessorDeclaration(member, ctx))
		} else if (nodeIs.isFunctionDeclaration(member)) {
			reflections.push(visitFunctionDeclaration(member, ctx))
		}
	}
