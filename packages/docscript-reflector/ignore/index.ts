...
export interface DocRegistry {
	...
	packages: { [pkg: string]: PackageInfo }
}
...
export class Context {
	modules: { [mod: string]: Module } = {}
	ts = typescript

	checker: TypeChecker | null = null
	program: Program

	mainPackage: string
	packages: { [mod: string]: Package } = {}
	...
	constructor(mainPackage: string, program: Program) {
	...
	processInternalModule(name: string, sourceFile: any, packageLike: boolean) {
		...
		let modulePath = path.join.apply(
			path,
			[
				pkg.path,
		...
		let module = this.generateModule(modulePath, sourceFile, pkg, true)
