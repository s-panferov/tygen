	...
	generateIdMap(): any[] {
		...
		function walkObject(
			...
			if (obj.selfRef && obj.itemType) {
				if (inMain && INCLUDE_ITEMS[obj.itemType]) {
					flatItems.push(`${pkg}://${path}#${obj.selfRef.semanticId}`)
				}
			}
		...
		Object.keys(modules).forEach(moduleKey => {
			...
			module.items.forEach(item => {
				walkObject(item, module.pkgName, module.fileInfo.relativeToPackage, inMain)
			})
		})
			...
			if (!shortForm) {
				...
				module.items.forEach(item => {
					let itemPath = path.join(
						itemsPath,
						(item.selfRef.semanticId || item.selfRef.id) + '.json'
					)
